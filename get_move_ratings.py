...
def get_move_constant(moveName):
    constant = moveName.upper().replace(" ", "_")
    return f"MOVE_{constant}"
...
def get_secondary_effect_rating(effect, self=False):
    ...
    if "status" in effect:
        ...
        if status in SECONDARY_EFFECT_STATUS_MODIFIERS:
            ...
            print(f"Warning: Unhandled status effect '{status}' ...")
    ...
    if "volatileStatus" in effect:
        ...
        if volatile_status in SECONDARY_EFFECT_STATUS_MODIFIERS:
            ...
            print(f"Warning: Unhandled volatile status effect '{volatile_status}' ...")
            effect_rating += SECONDARY_EFFECT_STATUS_MODIFIERS["default"]
...
if __name__ == "__main__":
    ...
    print(f"Filtering {len(MOVES)} moves ...")
    ...
    for moveId in moves:
        ...
        for flag in move["flags"]:
            if flag in MOVE_FLAG_EFFECT_MODIFIERS:
                ...
                print(f"Warning: Unhandled flag '{flag}' ...")
        ...
        if power == 0:  # Special case
            if moveId in ["fissure", "guillotine", "horndrill", "sheercold"]:
                ...
                print(f"Unhandled special case: {moveId}")
                power = 80
    ...
    for i in range(len(ratings_sorted)):
        ...
        output.append(f"\t[{get_move_constant(moveName)}] = {math.floor(move[1] * normalizer)},")
